...
class Learning(): 
    ...
    def scoreBtn(self, currentWindow):
        ...
        self.label5 = tkinter.Label(self.root, text = f"点数 : {self.score}",font=("times New Roman", 50))
        self.label5.place(x= 600, y = 300)

    def correctAnswer(self):
        self.label2 = tkinter.Label(self.root, text = "正解!!", font=("times New Roman", 60))
        self.label2.place(x =  610, y =  250)
        self.score += 1
        if self.number == 13:
            currentWindow = "正解"
            self.button5 = tkinter.Button(text = "score", command = lambda: self.scoreBtn(currentWindow))
            self.button5.place(x = 650, y = 500)
        else:
            self.button2 = tkinter.Button(text = "次へ", font=("times New Roman", 30),command = self.correctBtn)
            self.button2.place(x = 650, y = 500)
        
    def incorrectAnswer(self, correctAnswer):
        self.label3 = tkinter.Label(self.root, text = "残念 >< ", font=("System", 50))
        self.label3.place(x = 610, y = 200)
        self.label4 = tkinter.Label(self.root, text = f"正解は: {correctAnswer}", font=("System", 50))
